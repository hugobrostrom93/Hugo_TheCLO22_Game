...
void Shop()
{
    while (true)
    {
        ...
        if (shopItem == "3" && newPlayer.gold >= 400)
        {
            ...
            newPlayer.level += 2; // ge mig +2 lvlar för min LevelUp läser inte in rätt annars
            newPlayer.gold = newPlayer.gold - 400; // ge mig minus guld
            Console.WriteLine("You have " + newPlayer.gold + " gold");
...
// Monster objekt, Spelare objekt, minsta attack power för monster, max attack power för monster, guld vid dödat monster
void GameLoop(Monster monster, Player player, int min_enemy_attack_power, int max_enemy_attack_power, int lootGold)
{
    ...
    while (!player.IsDead && !monster.IsDead)
    {
        ...
        // skiver ut hp för spelare
        Console.WriteLine(player.name + ": " + player.hp + "hp");
        if (!monster.IsDead) // skadan skrivs bara ut om monstret dör så det inte står -X hp när det dör tex
        {
